...
{ toBe: (expected) => {
if ((expected !== result)) {
  console.log(`toBe fail: ${str(result)} = ${result}`);
}
}, toEqual: (expected) => {
if ((expected != result)) {
  console.log(`toEqual fail: ${str(result)} = ${result}`);
}
} }.toEqual([1, 2, 3]);
});
...
if ((expected !== executed)) {
  console.log(`toBe fail: ${str(executed)} = ${executed}`);
}
}, toEqual: (expected) => {
if ((expected != executed)) {
  console.log(`toEqual fail: ${str(executed)} = ${executed}`);
}
} }.toBe(false);
...
const result = run(from([1, 2, 3]), map((x) => {
(x * 2);
}), collect());
{ toBe: (expected) => {
if ((expected !== result)) {
...
const result = run(from([1, 2, 3, 4, 5]), filter((x) => {
((x % 2) === 0);
}), collect());
{ toBe: (expected) => {
if ((expected !== result)) {
...
const result = run(from([1, 2, 3, 4, 5]), takeWhile((x) => {
(x < 4);
}), collect());
{ toBe: (expected) => {
if ((expected !== result)) {
...
const result = run(from([1, 2, 3, 4, 5]), dropWhile((x) => {
(x < 3);
}), collect());
{ toBe: (expected) => {
if ((expected !== result)) {
...
const result = run(from([1, 2, 3]), flatMap((x) => {
Promise.resolve((x * 2));
}, {}), collect());
{ toBe: (expected) => {
if ((expected !== result)) {
...
let sideEffects = [];
const result = run(from([1, 2, 3]), tap((x) => {
...
const result = run(from([1, 2, 3, 4]), scan((acc, x) => {
(acc + x);
}, 0), collect());
{ toBe: (expected) => {
if ((expected !== result)) {
...
const result = run(from([1, 2, 3, 4]), reduce((acc, x) => {
(acc + x);
}, 0));
...
const result = run(from([1, 2, 3, 4]), some((x) => {
(x > 3);
}));
expect(result).toBe(true);
});
  it("every should return true if all match", async function() {
const result = run(from([2, 4, 6]), every((x) => {
((x % 2) === 0);
}));
expect(result).toBe(true);
});
...
const result = run(zip(from([1, 2, 3]), from(["a", "b", "c"])), collect());
{ toBe: (expected) => {
if ((expected !== result)) {
  console.log(`toBe fail: ${str(result)} = ${result}`);
}
...
const result = run(source, catchError((err) => {
return from([(0 - 1)]);
}), collect());
{ toBe: (expected) => {
if ((expected !== result)) {
...
const result = await run(range(0, 100, 1), filter((x) => {
((x % 2) === 0);
}), map((x) => {
(x * x);
}), take(5), scan((acc, x) => {
...
const result = run(from([[1, 2], [3, 4]]), flatMap((arr) => {
Promise.resolve(arr.reduce((a, b) => {
(a + b);
}));
}, {}), collect());
{ toBe: (expected) => {
...
const result = run(from([1, 2, 3, 4]), pairwise(), map((pair) => {
...
const result = run(from([1, 1, 2, 2, 2, 3, 3, 1]), distinctUntilChanged((a, b) => {
(a === b);
}), collect());
{ toBe: (expected) => {
if ((expected !== result)) {
...
const result = run(fromArray([1, 2, 3]), map((x) => {
(x * 2);
}), collect());
{ toBe: (expected) => {
if ((expected !== result)) {
...
const totalCount = run(range(0, 100000, 1), filter((x) => {
((x % 2) === 0);
}), map((x) => {
(x * 2);
}), count());
...
let activeItems = 0;
let maxActive = 0;
const result = run(range(0, 10000, 1), tap((_) => {
...
}, {}), count());
{
  ...
  expect((maxActive < 100)).toBe(true);
}
});
}
});
...
const source = from([1, 2, 3]);
{
  ...
  expect(isSource([1, 2, 3])).toBe(false);
