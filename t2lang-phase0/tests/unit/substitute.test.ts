...
function lit(n: number | string | null) { return { kind: "literal", value: n as any, location: loc } as Expr; }
...
test("substituteAndExpand replaces identifier bound in bindings", () => {
    ...
    const out = s.substituteAndExpand(id("x") as any, bindings) as any;
    assert.strictEqual(out.kind, "literal");
    assert.strictEqual((out as any).value, 42);
});
...
test("substituteInQuote handles ~ shorthand identifier", () => {
    ...
    const idTok = { kind: "identifier", name: "~a", location: loc } as any;
    const out = s.substituteInQuote(idTok as any, bindings) as any;
