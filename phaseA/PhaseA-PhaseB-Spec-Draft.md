...
- Phase B CLI extends `common/src/cliHelper.ts` with options for AST dumps before/after macros, rewrite maps, tracing, and seeding (`--ast-before-expand`, `--ast-after-expand`, `--trace`, `--seed`, `--log-level`).
- Diagnostic formatting aligns with TypeScript errors when possible but prioritizes Phase B sugar contexts (macro invocations, quasiquoted forms, operator rewrites).
- The CLI handles `t2tc`, `t2jc`, and `t2run` entry points, reusing shared helpers while adding Phase B-specific behavior (macros, rewrite diagnostics, Prettier formatting).
