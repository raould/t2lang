...
test("macro generates new expression", async () => {
  const result = await compilePhase1(`
    (program
      ...
      (mknew Foo))
  `, { enableTsc: false });
  expectNoErrors(result, "mknew"); // Error logging added
...
// New tests: nested splices, gensym uniqueness, unquote-splice non-array handling, nested splice in nested array

test("nested unquote-splice in call args", async () => {
  const result = await compilePhase1(`
    (program
      (defmacro mkcall (a b) (quote (call foo ~@a ~@b)))
      (call (prop console "log") (mkcall (array 1 2) (array 3 4))))
  `, { enableTsc: false });
  expectNoErrors(result, "mkcall");
  // args should be flattened into 1,2,3,4
  assert(result.tsSource.includes("1, 2, 3, 4") || result.tsSource.includes("1,2,3,4"));
});
...
test("multiple gensyms produce unique names", async () => {
  ...
  `, { enableTsc: false });
  expectNoErrors(result, "twoG");
  const matches = result.tsSource.match(/x\d+/g) || [];
  // Should find at least two matches and they should be distinct
...
test("unquote-splice with non-array treated as single value", async () => {
  const result = await compilePhase1(`
    (program
      (defmacro s (x) (quote (array ~@x)))
      (call (prop console "log") (s 5)))
  `, { enableTsc: false });
  expectNoErrors(result, "s");
  // should expand to [5]
  assert(result.tsSource.includes("[5") || result.tsSource.includes("array [5") || result.tsSource.includes("array[5"));
});

test("nested splice inside nested array expands correctly", async () => {
  const result = await compilePhase1(`
    (program
      (defmacro mknewargs (a) (quote (new Foo (array 0 ~@a 9))))
      (call (prop console "log") (mknewargs (array 1 2 3))))
  `, { enableTsc: false });
  expectNoErrors(result, "mknewargs");
  assert(result.tsSource.includes("0, 1, 2, 3, 9") || result.tsSource.includes("0,1,2,3,9"));
});
