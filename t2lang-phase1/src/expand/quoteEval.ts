...
import { QuotedToAstConverter, GensymGenerator, Substitutor } from "t2lang-phase0";
...
export class QuoteEvaluator {
  ...
  convertQuotedToAstSingle(node: Expr | SpliceMarker): any {
    const r = this.quotedConverter.convertQuotedToAst(node as any);
  ...
  flattenQuotedArgs(nodes: Array<Expr | SpliceMarker>): any[] {
    return this.quotedConverter.flattenQuotedArgs(nodes as any);
  }

  convertQuotedToAst(expr: Expr | SpliceMarker): Expr | SpliceMarker {
    return this.quotedConverter.convertQuotedToAst(expr as any) as any;
  }
  ...
  evalMacroExpr(expr: Expr, env: Map<string, Expr>): Expr {
    switch (expr.kind) {
      ...
      default:
        return this.substitutor.substituteAndExpand(expr as any, env) as Expr;
    }
  }
