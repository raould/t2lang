  ...
  if ((sfNode.tag === "sf-list")) {
    {
      ...
      items.forEach((item) => {
        if (((item.tag === "sf-splice") && (innerDepth === 1))) {
          ...
          {
            result.push(evalQuasiToSForm(item, bindings, env, innerDepth));
          }
        }
      });
      return result;
...
const applyMacroFn  = (fnNode, evArgs, env) => {
  {
        ...
        {
          {
            ...
            if ((Array.isArray(arr) && (cbFn && cbFn.params))) {
              return arr.map((item) => {
                return applyMacroFn(cbFn, [item], env);
              });
            }
            else {
              return [];
