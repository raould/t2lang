...
- See `phaseA/PhaseA0-Calculus.md` for the precise calculus that Phase A0 exposes.
...
4. **Async-by-default helpers** (future work): we also intend to explore ultra-compact sugar for asynchronous code—Phase B should eventually offer terse `async` function literals plus paired `await` call aliases so writing async-first programs feels as natural as the rest of the ergonomic syntax. The precise keyword shapes and rewrite rules are TBD, but we expect Phase B to rewrite those helpers into the established `FunctionExpr`/`AwaitExpr` nodes before handing the AST to Phase A so the core remains unchanged.
