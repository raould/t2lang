...
## Goal 🎯
Make `phase1` a **thin wrapper** around `phase0` so Phase1 contains only the minimal code needed to:
...
1. **Inventory**: Find duplicated files and APIs across `phase1/src` and `phase0/src`.
...
Action: revert manual edits in dist/src/*.js so they are generated by tsc; run npm -w common run build and npm -w phase1 run build to regenerate artifacts. If you must keep manual run-time fixes, prefer editing the TypeScript source and then rebuilding.
...
Action: finalize package.json exports and index.d.ts; consider adding types to package then switch phase1 to static imports.
...
- Sexpr printer consolidated in `common`:
  ...
  - `phase1` now statically imports the printer from `t2lang-common/ast/sexprPrinter.js` (no runtime fallback).
...
- Tests (local run):
  - `phase0` tests: **237** passing, **0** failing.
  - `phase1` tests: **59** passing, **0** failing.
...
- Extract a well-defined helper into `phase0/src/lib` or a new `common` package (e.g., `parseQuotedSexpr`, `normalizeQuotedAst`, `expandGensym`, `spliceMarker` handling).
...
## Testing Strategy 🧪
- Add regression tests to `phase1/tests/integration/examples` for all previously problematic macros (done already for many cases).
...
npm i ../common/t2lang-common-0.1.0.tgz && npm run -w phase1 test
Test TypeScript resolution:
From a TypeScript consumer, run tsc --noEmit or npm run -w phase1 build to ensure types resolve through the installed package.
