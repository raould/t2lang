...
let emitFieldDef  = (node) => {
  {
    ...
    return (((node.name + typeStr) + initStr) + ";");
  }
};
...
let emitClassMethodDef  = (node) => {
  {
    ...
    let body  = node.body.map(emitStmt);
    return (((((((node.name + "(") + paramsStr) + ")") + returnTypeStr) + " {\n") + indent(body.join("\n"))) + "\n}");
  }
};
let emitAbstractMethodDef  = (node) => {
  {
    ...
    let paramsStr  = params.join(", ");
    let returnTypeStr  = (node.sig.returnType ? (": " + emitTypeExpr(node.sig.returnType)) : "");
    return (((((("abstract " + node.name) + "(") + paramsStr) + ")") + returnTypeStr) + ";");
  }
};
...
let emitClassDef  = (node) => {
  {
    ...
    return ((((("class " + node.name) + typeParamsStr) + " {\n") + indent(bodyStr)) + "\n}");
  }
};
