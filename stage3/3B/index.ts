          ...
          import fs from 'node:fs';;
{
  ...
  let astStatement = lg("astStatement", (ctx) => {
    if (ctx.letStar()) {
  ...
  let astCondExpr = (ctx) => {
    {
      ...
      return { tag: 'cond', clauses };
    }
  };
  let astNewExpr = (ctx) => {
    {
      let clazz = ctx.IDENTIFIER().getText();
      return { tag: 'new', clazz, args: ctx.expression().map(e => astExpression(e)) };
    }
  };
  let astOptChain = (ctx) => {
    {
  ...
  let astNullCoalesce = (ctx) => {
    {
      ...
      return { tag: 'null-coalesce', left, right };
    }
  };
  let astExpression = lg("astExpression", (ctx) => {
    if ((ctx == undefined)) {
  ...
  let astPropAccess = (ctx) => {
    {
      ...
      return { tag: 'prop-access', object: obj, key };
    }
  };
  ...
  let lowerStmt = lg("lowerStmt", (node) => {
    if ((node.tag === "let*")) {
  ...
  let lowerExpr = lg("lowerExpr", (node) => {
    ...
    if ((node.tag === "new")) {
      return { tag: 'new-expr', clazz: node.clazz, args: node.args.map(lowerExpr) };
    }
  ...
  let main = () => {
    {
      ...
      let lexer = new Stage3BLexer(inputStream);
      let tokenStream = new CommonTokenStream(lexer);
      let parser = new Stage3BParser(tokenStream);
