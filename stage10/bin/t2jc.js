...
import { compile, compileSource } from '../index.ts';
import { makeDebugContextFromOptions } from '../src/Stage10-debug.ts';
...
function runTsc(tsPath) {
  ...
  let tempTsconfig;
  if (tsconfig) {
    tempTsconfig = resolve(tmpdir(), `t2jc_${Date.now()}.tsconfig.json`);
