...
import { makeMacroEnv } from '#src/Stage10-macro-env';
import { evalQuasi, extractBindingName } from '#src/Stage10-macro-expand';
...
describe('evalQuasi: if node', () => {
  ...
  it('substitutes unquote in ifthen and ifelse', () => {
    ...
    const result = evalQuasi(node, bindings, env, 1);
    expect(result.ifthen[0]).toBe(thenNode);
    expect(result.ifelse[0]).toBe(elseNode);
  });
