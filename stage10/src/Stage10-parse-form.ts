// Stage10-parse-form.ts — hand-authored
// Converts an SForm (or SFormNode) to a typed AST node.
//
// Called by Stage10-macro-expand (after evalQuasiToSForm) and Stage10-lower
// (directly with the grammar-produced SFormNode).

import { nextNodeId } from "./Stage10-spans";
...
function parseTypedParam(sform: any, spanId: number, env: any): any {
  if (isOpaque(sform)) {
    const inner = sform.node;
    // (quasi (~x)) where x is an identifier produces a zero-arg call node.
    // In lambda params position, treat it as a plain param name.
    if (inner && inner.tag === "call" && inner.args && inner.args.length === 0
        && inner.fn && inner.fn.tag === "identifier") {
      const name = inner.fn.name;
      return { name, optional: false, typeAnnotation: null, id: nextNodeId() };
    }
    // Opaque wrapping an identifier node (e.g. from ~param when param is an identifier)
    if (inner && inner.tag === "identifier") {
      return { name: inner.name, optional: false, typeAnnotation: null, id: nextNodeId() };
    }
    return inner;
  }
  if (typeof sform === "string") {
...
function parseFnSig(
  ...
  // Handle opaque-wrapped call node from ~params quasi splice:
  // (x y z) parsed as call(x,[y,z]) → extract all idents as param forms
  if (!Array.isArray(sigSform) && isOpaque(sigSform)) {
    const n = sigSform.node;
    if (n && n.tag === "call") {
      paramForms = [n.fn, ...n.args].map((a: any) => ({ __opaque: true, node: a }));
    } else if (n && n.tag === "identifier") {
      paramForms = [{ __opaque: true, node: n }];
    }
  }
  const params: any[] = [];
  let rest: any = null;
...
function parseFormImpl(sform: any, spanId: number, env: any): any {
  ...
  if (typeof sform === "string") {
    ...
    // Dotted atom in quasi sform: "obj.prop.sub" → nested prop-access nodes
    if (sform.includes(".") && !sform.startsWith(".") && !sform.endsWith(".")) {
      const parts = sform.split(".");
      let node: any = mk("identifier", { name: parts[0] });
      for (let di = 1; di < parts.length; di++) {
        node = mk("prop-access", { object: node, key: parts[di] });
      }
      return node;
    }
  ...
  if (headStr === "switch") {
    // (switch discriminant (case val body...) ... (default body...))
    const discriminant = parseFormImpl(tail[0], spanId, env);
    const cases: any[] = [];
    let defaultCase: any = undefined;
    for (const clause of tail.slice(1)) {
      // Opaque wrapping a call node with fn.name "case"/"default":
      // produced when (quasi (case ~i ~@body)) is evaluated — parseForm turns ["case", ...]
      // into a call AST node, then nodeToSForm wraps it as opaque for the outer switch splice.
      // Note: unlike the plain-array path below, bodies here are already-parsed AST nodes
      // (not SForms), so they are used as-is without re-parsing via parseFormImpl.
      if (isOpaque(clause)) {
        const n = clause.node;
        if (n && n.tag === "call" && n.fn && n.fn.tag === "identifier") {
          const fname = n.fn.name;
          if (fname === "case") {
            const test = n.args[0]; // already a parsed AST node
            const body = n.args.slice(1); // already parsed AST nodes
            cases.push({ test, body });
            continue;
          } else if (fname === "default") {
            defaultCase = { body: n.args }; // already parsed AST nodes
            continue;
          }
        }
        continue;
      }
      const arr = Array.isArray(clause) ? clause : null;
      if (!arr) continue;
      const clauseHead = typeof arr[0] === "string" ? arr[0]
        : isOpaque(arr[0]) ? (arr[0].node?.name ?? arr[0].node?.value) : null;
      if (clauseHead === "case") {
        const test = parseFormImpl(arr[1], spanId, env);
        const body = arr.slice(2).map((s: any) => parseFormImpl(s, spanId, env));
        cases.push({ test, body });
      } else if (clauseHead === "default") {
        const body = arr.slice(1).map((s: any) => parseFormImpl(s, spanId, env));
        defaultCase = { body };
      }
    }
    return mk("switch", { discriminant, cases, defaultCase });
  }
  ...
  // Dotted-sugar inside quasi: "obj." as head with key as sole arg (from __inst.~key in sform)
  if (headStr !== null && headStr.endsWith(".") && headStr.length > 1 && tail.length === 1) {
    const objName = headStr.slice(0, -1);
    const keyArg = tail[0];
    const keyNode = isOpaque(keyArg) ? keyArg.node : parseFormImpl(keyArg, spanId, env);
    const key = keyNode && keyNode.tag === "identifier" ? keyNode.name : typeof keyArg === "string" ? keyArg : "?";
    return mk("prop-access", { object: mk("identifier", { name: objName, id: nextNodeId() }), key });
  }
  ...
  // ---- Operator forms ----
  // and/or as short-circuit logical operators
  if ((headStr === "and" || headStr === "or") && tail.length >= 1) {
    const jsOp = headStr === "and" ? "&&" : "||";
    let left = parseFormImpl(tail[0], spanId, env);
    for (let li = 1; li < tail.length; li++) {
      const right = parseFormImpl(tail[li], spanId, env);
      left = mk("binary-op", { op: jsOp, left, right });
    }
    return left;
  }

  const BINARY_OPS = new Set(["===", "!==", "==", "!=", ">=", "<=", "<", ">", "&&", "||", "+", "-", "*", "/", "%", "&", "|", "^", "**"]);
  if (headStr !== null && BINARY_OPS.has(headStr) && tail.length >= 2) {
    // Fold: (op a b c) → binary-op(op, binary-op(op, a, b), c)
    let left = parseFormImpl(tail[0], spanId, env);
    for (let bi = 1; bi < tail.length; bi++) {
      const right = parseFormImpl(tail[bi], spanId, env);
      left = mk("binary-op", { op: headStr, left, right });
    }
    return left;
  }
  if (headStr === "!" && tail.length === 1) {
    return mk("unary-op", { op: "!", operand: parseFormImpl(tail[0], spanId, env) });
  }
  if (headStr === "typeof" && tail.length === 1) {
    return mk("unary-op", { op: "typeof", operand: parseFormImpl(tail[0], spanId, env) });
  }
