...
function genStatement(stmt: Statement, options: TsCodegenOptions, builder: TsBuilder): void {
  switch (stmt.kind) {
    ...
    case "block":
      ...
      break;
    case "let":
      builder.emitWithMapping(genLet(stmt as LetExpr, options), stmt.location);
      break;
    case "type-alias":
...
function genExpr(expr: Expr, options: TsCodegenOptions): string {
  switch (expr.kind) {
    ...
    case "let":
      return genLet(expr as LetExpr, options);
    ...
    case "gensym":
      return genGensym(expr as GensymExpr);
...
function genGensym(node: GensymExpr): string {
  // After macro expansion, gensyms should be converted to identifiers
  // If we get here, it means gensym was used outside a macro context
  // Generate the identifier name directly
  if (node.generatedName) {
    return node.generatedName;
  }
  // Fallback: generate a name (shouldn't happen after proper expansion)
  const prefix = node.prefix || "G__";
...
function genLet(node: LetExpr, options: TsCodegenOptions): string {
  let result = "{\n";
