...
let emitAbstractMethodDef  = (node) => {
  {
    let params  = node.sig.params.map(emitTypedParam);
    let paramsStr  = params.join(", ");
    let returnTypeStr  = (node.sig.returnType ? (": " + emitTypeExpr(node.sig.returnType)) : "");
...
let emitClassElement  = (node) => {
  ...
  throw new Error((("emitClassElement: unexpected tag >" + node.tag) + "<"));
};
let emitClassBody  = (body) => {
  {
    let elements  = body.elements.map(emitClassElement);
    return elements.join("\n");
  }
};
let emitClassDef  = (node) => {
  {
...
let emitExportDefaultClass  = (node) => {
  ...
  throw new Error((("emitExportDefaultClass: unexpected classNode tag >" + node.classNode.tag) + "<"));
};
let emitExportDefaultFnDecl  = (node) => {
  {
    let params  = node.params.join(", ");
    let body  = node.body.map(emitStmt);
