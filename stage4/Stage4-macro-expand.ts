import { gensym, freshScope } from "./Stage4-macro-env";
...
let expandMacroCall  = (callNode, env) => {
  {
    ...
    if ((argNodes.length !== macroDef.params.length)) {
      {
        env.errors.push(({
          kind: "arity",
          message: ((((macroName + " expects ") + macroDef.params.length) + " args, got ") + argNodes.length),
          macroName: macroName,
          callSite: callSite
        }));
        return callNode;
...
let formatExpansionErrors  = (errors) => {
  {
    ...
    });
    {
      ...
      grouped.forEach((errs, nm) => {
        ...
        errs.forEach((err) => {
          {
            let loc  = (err.callSite ? ((" (at: " + err.callSite) + ")") : "");
