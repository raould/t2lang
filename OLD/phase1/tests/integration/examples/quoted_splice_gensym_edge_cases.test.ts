...
function expectNoErrors(result: unknown, name: string) {
  const r = result as { errors?: unknown[]; tsSource?: string };
  const errs = Array.isArray(r.errors) ? r.errors : [];
  if (errs.length > 0) {
    console.log(`DEBUG ${name} errors:`, JSON.stringify(errs, null, 2));
