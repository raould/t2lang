...
test("type parser reports unexpected trailing token as E110", () => {
  assert.throws(
    ...
    (error) => error instanceof T2CompilationError && error.code === "T2:0110" && error.message.includes("foo")
  );
});
...
test("type parser reports missing identifier after typeof as E111", () => {
  assert.throws(
    ...
    (error) => error instanceof T2CompilationError && error.code === "T2:0111"
  );
});
...
test("type parser reports missing closing bracket as E112", () => {
  assert.throws(
    ...
    (error) => error instanceof T2CompilationError && error.code === "T2:0112" && error.message.includes("]")
  );
});
...
test("expander surfaces unsupported macro substitution as E101", () => {
  ...
  assert.throws(
    ...
    (error) => error instanceof T2CompilationError && error.code === "T2:0101"
  );
});
...
test("expander surfaces unsupported unquote-splicing as E102", () => {
  ...
  assert.throws(
    ...
    (error) => error instanceof T2CompilationError && error.code === "T2:0102"
  );
});
